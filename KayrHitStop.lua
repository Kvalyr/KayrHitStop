--[[
	...
	function KayrHitStop:Setup()
		if KayrHitStop.ConfigValid then
			...
			KayrHitStop.cfgScheme:NewElement("minimumHitFrameRateMultiplier", 0.5, KayrHitStop.UpdateConfigvalues, "class", "class")
			...
			KayrHitStop.cfgScheme:NewElement("baseHitstopDelay", 0.125, KayrHitStop.UpdateConfigvalues, "class", "class")
	...
	function KayrHitStop:COMBAT_LOG_EVENT_UNFILTERED(event, ...)
		...
		local timestamp, event_type, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, spellName = CombatLogGetCurrentEventInfo()
		
		-- Filter by caster - We only care about the player's actions
		if sourceName ~= UnitName("player") then return end
		
		--KayrHitStop:Con("HitStop event:", event_type, spellName)
		...
		if not KayrHitStop.hitStopEvents[event_type] then 
			--KayrHitStop:Con("HitStop skipped event:", event_type, spellName)
			return 
		end
		
		-- Filter out spells that are too spammy or don't feel right with hitstops
		if KayrHitStop.ignoredSpells and KayrHitStop.ignoredSpells[spellName] then return end
		...
		if KayrHitStop.cfgScheme:Get("spellBookOnly") and not (GetSpellBookItemInfo(spellName) or IsTalentSpell(spellName)) then
			--KayrHitStop:Con("Skipping non-spellbook ability:", spellName)
			return
		end		
		...
		if meleeOnly and (minRange ~= 0 or maxRange ~= 0) then 
			--KayrHitStop:Con("HitStop skipped non-melee:", event_type, spellName)
			return 
		end
		...
		if ignoreAoE and not KayrHitStop:tobool(IsSpellInRange(spellName)) then 
			--KayrHitStop:Con("HitStop skipped AoE:", event_type, spellName)
			return
		end
		
		KayrHitStop:HitStop(timestamp, event_type)
	end
